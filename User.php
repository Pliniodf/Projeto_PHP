...
class User extends Conn
{
    ...
    public function list() :array
    {
        ...
        $query_users = "SELECT id, nome, email users FROM users ORDER BY id DESC LIMIT 40";
    ...
    {
        $this->conn = $this->connectDb();
        $query_user = "SELECT id, nome, email, created, midifield FROM users WHERE id =:id LIMIT 1";
    ...
    public function edit() : bool
    {
        //var_dump($this->formData);
        $this->conn = $this->connectDb();
        $query_user = "UPDATE users SET nome=:nome, email=:email, midifield=NOW() 
                    WHERE id=:id";
